...
For an explicitly requested one-time catalogue expansion, `scripts/backfill_library.py` uses the same public source and the same verified importer, but samples separate tag feeds for all seven Cosmic Aquaria waters. It fills the least represented water first, saves after every successful Aquarium, and can resume without duplicating releases. This does not change the normal daily target of twenty.
