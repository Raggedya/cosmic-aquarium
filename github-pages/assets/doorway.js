(() => {
  ...
  if (!matchMedia('(prefers-reduced-motion: reduce)').matches) {
    const bodies = bubbles.map((node,index)=>({node,x:0,y:0,vx:Math.sin(index*2.31)*.018,vy:Math.cos(index*1.73)*.014,phase:index*1.41,radius:node.offsetWidth*.43,cx:0,cy:0,anchor:node.dataset.water==='anywhere'}));
    const measure = () => { const f=field.getBoundingClientRect(); bodies.forEach(body=>{const r=body.node.getBoundingClientRect();body.cx=r.left+r.width/2-f.left-body.x;body.cy=r.top+r.height/2-f.top-body.y;body.radius=r.width*.43}) };
    measure(); let previous=performance.now();
    const tick = now => { const dt=Math.min(2,Math.max(.5,(now-previous)/16.67));previous=now;bodies.forEach((body,index)=>{const spring=body.anchor?.0018:.0008,amplitude=body.anchor?.0008:.0018;body.vx+=(Math.sin(now*.00023+body.phase)*amplitude-body.x*spring)*dt;body.vy+=(Math.cos(now*.00019+body.phase*1.7)*amplitude-body.y*spring)*dt;body.vx*=Math.pow(.986,dt);body.vy*=Math.pow(.986,dt);const limit=body.anchor?2.5:8;body.x=Math.max(-limit,Math.min(limit,body.x+body.vx*dt));body.y=Math.max(-limit,Math.min(limit,body.y+body.vy*dt));for(let j=index+1;j<bodies.length;j++){const other=bodies[j],dx=other.cx+other.x-body.cx-body.x,dy=other.cy+other.y-body.cy-body.y,distance=Math.max(1,Math.hypot(dx,dy)),overlap=body.radius+other.radius+3-distance;if(overlap>0){const pressure=Math.min(.012,overlap*.00045),nx=dx/distance,ny=dy/distance;body.vx-=nx*pressure;body.vy-=ny*pressure;other.vx+=nx*pressure;other.vy+=ny*pressure}}body.node.style.setProperty('--drift-x',body.x.toFixed(2)+'px');body.node.style.setProperty('--drift-y',body.y.toFixed(2)+'px')});requestAnimationFrame(tick)};
