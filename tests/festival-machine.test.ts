...
test('Festival Mode owns the final timber instrument cabinet, wide reel and round control bank',async()=>{
  ...
  assert.match(css,/machine-controls>\.secondary-button,[^}]*machine-controls>\.buy-button\{[^}]*aspect-ratio:1[^}]*align-self:center[^}]*justify-self:center[^}]*font-size:clamp\(9px,2\.5vw,13px\)/);
  assert.match(css,/machine-controls>\.buy-button>span\{[^}]*transform:none[^}]*line-height:\.92/);
  assert.match(css,/machine-controls>\.re-spin-button\{[^}]*font-size:clamp\(9px,2\.5vw,13px\)[^}]*letter-spacing:-\.015em/);
  assert.match(css,/machine-controls>\.love-button\{[^}]*flex-direction:column[^}]*gap:\.18em/);
