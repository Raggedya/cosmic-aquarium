const EVENT_TYPES = new Set([
  ...
  'artist_machine_loaded','festival_machine_loaded','festival_website_click','artist_link_click','reel_spin','spin_started','spin_completed','re_spin',
...
const EVENT_LABELS = {
  ...
  artist_machine_loaded:'Artist machines opened',festival_machine_loaded:'Festival machines opened',festival_website_click:'Festival website clicks',artist_link_click:'Festival artist link clicks',
...
async function randomCollectionArtist(url, env, slug) {
  ...
  const requestedWater = clean(url.searchParams.get('water'),24)?.toLowerCase() || null;
  const water = requestedWater && WATERS.has(requestedWater) ? requestedWater : null;
  const recent = (url.searchParams.get('recent') || '').split(',').map(value=>value.trim()).filter(Boolean).slice(0,8);
  const exclusions = [exclude,...recent].filter(Boolean);
  const exclusionSql = exclusions.length ? ` AND ca.artist_id NOT IN (${exclusions.map(()=>'?').join(',')})` : '';
  const entry = await env.DB.prepare(`SELECT ar.primary_aquarium_id AS id,a.slug,a.artist,a.release_title,a.aquarium_url,ca.artist_id
    ...
    WHERE c.slug=? AND c.status='published' AND ca.display_enabled=1
      ...
      AND (? IS NULL OR EXISTS (SELECT 1 FROM aquarium_water aw WHERE aw.aquarium_id=ar.primary_aquarium_id AND aw.water=?))
      ${exclusionSql}
    ORDER BY RANDOM() LIMIT 1`)
    .bind(slug,water,water,...exclusions).first();
...
async function syncCatalogue(request, env) {
  ...
  for (const item of artists) {
    ...
    statements.push(env.DB.prepare(`INSERT INTO artist
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(id) DO UPDATE SET name=excluded.name,canonical_name=excluded.canonical_name,bandcamp_artist_url=excluded.bandcamp_artist_url,primary_aquarium_id=excluded.primary_aquarium_id,status=excluded.status,updated_at=excluded.updated_at,display_name=excluded.display_name,slug=excluded.slug,primary_location_id=excluded.primary_location_id,country=excluded.country,metadata=excluded.metadata`)
      .bind(item.id,item.name||'',item.canonicalName||String(item.name||'').toLowerCase(),item.bandcampArtistUrl,null,item.status==='disabled'?'disabled':'published',item.createdAt||now,now,item.name||'',item.aquariumSlug||null,primaryLocationMembership?.id||null,String(item.primaryLocation||'').split(',').at(-1)?.trim()||null,JSON.stringify({waters:item.waters||[],labels:item.labels||[],memberships:item.memberships||[],release:item.release||null,releaseDate:item.releaseDate||null,trackCount:item.trackCount||0})));
  }
  ...
  for (const item of collections) {
    ...
    for (const member of Array.isArray(item.members)?item.members:[]) {
      ...
      statements.push(env.DB.prepare(`INSERT OR REPLACE INTO collection_artist
        (collection_id,artist_id,verification_status,verification_score,source,evidence,display_enabled,added_at,confidence,membership_status,administrator_override,updated_at)
...
async function activityReport(env, endValue = Date.now()) {
  ...
  const festivalMachines = await env.DB.prepare(`SELECT substr(aquarium_id,18) AS slug,
    COALESCE(MAX(NULLIF(json_extract(metadata,'$.festivalName'),'')),substr(aquarium_id,18)) AS festival_name,
    COUNT(CASE WHEN event_type='festival_machine_loaded' THEN 1 END) AS opens,
    COUNT(DISTINCT CASE WHEN session_id<>'system' THEN session_id END) AS visitors,
    COUNT(CASE WHEN event_type='spin_started' THEN 1 END) AS spins,
    COUNT(DISTINCT CASE WHEN event_type='winner_revealed' THEN NULLIF(json_extract(metadata,'$.artist'),'') END) AS artists_surfaced,
    COUNT(CASE WHEN event_type='track_play_started' THEN 1 END) AS tracks_played,
    COUNT(CASE WHEN event_type='bandcamp_click' THEN 1 END) AS bandcamp_clicks,
    COUNT(CASE WHEN event_type='buy_click' THEN 1 END) AS buy_clicks,
    COUNT(CASE WHEN event_type='artist_link_click' THEN 1 END) AS artist_link_clicks,
    COUNT(CASE WHEN event_type='share_click' THEN 1 END) AS shares
    FROM analytics_event WHERE created_at>=? AND created_at<? AND aquarium_id LIKE 'festival-machine:%'
    GROUP BY slug ORDER BY visitors DESC,opens DESC,festival_name COLLATE NOCASE`)
    .bind(window.start,window.end).all();
  const festivalArtists = await env.DB.prepare(`SELECT substr(aquarium_id,18) AS slug,
    COALESCE(NULLIF(json_extract(metadata,'$.artist'),''),NULLIF(json_extract(metadata,'$.artistName'),''),'Unknown artist') AS artist,
    COUNT(CASE WHEN event_type='winner_revealed' THEN 1 END) AS discoveries,
    COUNT(CASE WHEN event_type='track_play_started' THEN 1 END) AS plays,
    COUNT(CASE WHEN event_type IN ('bandcamp_click','artist_link_click') THEN 1 END) AS clicks
    FROM analytics_event WHERE created_at>=? AND created_at<? AND aquarium_id LIKE 'festival-machine:%'
      AND event_type IN ('winner_revealed','track_play_started','bandcamp_click','artist_link_click')
    GROUP BY slug,artist ORDER BY slug,discoveries DESC,plays DESC,clicks DESC,artist COLLATE NOCASE`)
    .bind(window.start,window.end).all();
  const artistsByFestival=new Map();
  for(const item of festivalArtists.results||[]){const artists=artistsByFestival.get(item.slug)||[];if(artists.length<5)artists.push({...item,discoveries:Number(item.discoveries||0),plays:Number(item.plays||0),clicks:Number(item.clicks||0)});artistsByFestival.set(item.slug,artists)}
  ...
  return {...window,uniqueSessions:Number(visitors?.total||0),totals,
    ...
    festivalMachines:(festivalMachines.results||[]).map(item=>({...item,
      opens:Number(item.opens||0),visitors:Number(item.visitors||0),spins:Number(item.spins||0),artists_surfaced:Number(item.artists_surfaced||0),tracks_played:Number(item.tracks_played||0),bandcamp_clicks:Number(item.bandcamp_clicks||0),buy_clicks:Number(item.buy_clicks||0),artist_link_clicks:Number(item.artist_link_clicks||0),shares:Number(item.shares||0),topArtists:artistsByFestival.get(item.slug)||[],
    })),
...
function activityReportMessage(report) {
  ...
  const festivalMachineRows=(report.festivalMachines||[]).map(item=>{
    const artists=(item.topArtists||[]).map(artist=>`${escapeHtml(artist.artist)} (${Number(artist.discoveries||0)} discoveries, ${Number(artist.plays||0)} plays, ${Number(artist.clicks||0)} clicks)`).join(' · ');
    return `<tr><td style="padding:10px;border-bottom:1px solid #272940">${escapeHtml(item.festival_name)}<br><span style="color:#777386;font-size:10px">${artists||'No artist activity yet'}</span></td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.opens||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.visitors||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.spins||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.artists_surfaced||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.tracks_played||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.bandcamp_clicks||0)}</td><td style="padding:10px;border-bottom:1px solid #272940;text-align:center">${Number(item.shares||0)}</td></tr>`;
  }).join('');
  const festivalSection=`<h2 style="font-size:18px;font-weight:400;margin-top:30px">Festival Machine performance</h2><p style="color:#aaa4b7;font-size:12px">Anonymous opens, visitors, spins, artists surfaced, plays, Bandcamp intent and shares for each active festival.</p><div style="overflow-x:auto"><table role="presentation" style="width:100%;min-width:860px;border-collapse:collapse;background:#0a0d20"><thead><tr style="color:#aaa4b7;font-size:10px"><th style="padding:9px;text-align:left">FESTIVAL / LEADING ARTISTS</th><th>OPENS</th><th>VISITORS</th><th>SPINS</th><th>ARTISTS</th><th>PLAYS</th><th>BANDCAMP</th><th>SHARES</th></tr></thead><tbody>${festivalMachineRows||'<tr><td colspan="8" style="padding:10px">No Festival Machine activity yet.</td></tr>'}</tbody></table></div>`;
  ...
  const festivalText=['FESTIVAL MACHINE PERFORMANCE',...(report.festivalMachines||[]).map(item=>`${item.festival_name}: opens ${item.opens}, visitors ${item.visitors}, spins ${item.spins}, artists surfaced ${item.artists_surfaced}, plays ${item.tracks_played}, Bandcamp ${item.bandcamp_clicks}, artist links ${item.artist_link_clicks}, buy ${item.buy_clicks}, shares ${item.shares}; leading artists ${(item.topArtists||[]).map(artist=>`${artist.artist} (${artist.discoveries} discoveries, ${artist.plays} plays, ${artist.clicks} clicks)`).join('; ')||'none'}`),''].join('\n');
  return {html:html.replace('<h2 style="font-size:18px;font-weight:400;margin-top:30px">All activity types</h2>',festivalSection+'<h2 style="font-size:18px;font-weight:400;margin-top:30px">All activity types</h2>'),text:text.replace('\nALL ACTIVITY TYPES',`\n${festivalText}\nALL ACTIVITY TYPES`)};
}
